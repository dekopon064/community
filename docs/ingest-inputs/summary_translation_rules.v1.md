...
- 원문에 없는 연도를 추측하거나 보충하지 않는다.
- 원문에 월·일만 있으면 결과에도 연도를 추가하지 않는다.
- `2024년(연도 미기재)` 또는 `2024年(年度未記載)` 같은 표현을 만들지 않는다.
- 연도를 알 수 없으면 월·일 정보만 자연스럽게 작성한다.
- 한국어 결과에서는 원문의 `~` 범위를 보존한다. 예: `월~금 10:00~22:00`
- 일본어 결과에서는 범위에 `～`를 사용한다.
- 날짜와 시간의 시작·종료 값을 생략하거나 붙여 쓰지 않는다.

## Currency

- 한국 원화의 숫자와 단위를 정확히 보존한다.
- 한국 원화는 반드시 `ウォン`으로 쓴다.
- `10만원 → 100,000ウォン`
- `20만 원 → 200,000ウォン`
- `1천원 → 1,000ウォン`
- `1,000원 → 1,000ウォン`
- `만`을 천 단위로 잘못 변환하지 않는다.
- 금액을 엔화로 환산하지 않는다.
- `円`, `￥`, `¥`를 사용하지 않는다.
- 통화를 환산하지 않는다.
...
- 기존 승인 용어집 `region_ja_glossary.v1`을 우선 사용한다.
...
- 시·군·구도 지역 고유명만 가타카나로 쓰고 행정 단위는 `市`, `郡`, `区`로 쓴다.
  ...
  - `강남구 → カンナム区`
- `경기광주시 → キョンギ道クァンジュ市`
- `경기`가 경기도 하위 시·군 앞에 붙으면 `キョンギ道`로 표현한다.
- `제주 표선 → チェジュ・ピョソン`
- 용어집에 없는 고유 지역명은 한국어 발음을 기준으로 가타카나로 음역한다.
- `表善(ピョソン)`, `京畿道（キョンギド）`처럼 한자명 뒤에 가타카나 독음을 괄호로 붙이지 않는다.
- 한국어 원명과 괄호 독음을 일본어 결과에 병기하지 않는다.
- 괄호 독음을 추가하지 않는다.
