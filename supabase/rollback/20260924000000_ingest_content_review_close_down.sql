-- Restore content-review close to the prior region/relevance decision writer
-- and the prior evaluation writer. Does not delete decision, job, or candidate rows.
-- Fails closed when a content decision already exists.
...
begin
  if exists (
    ...
    where d.review_type = 'content'
  ) then
    raise exception 'rollback_content_review_close_data_present';
  end if;
end
$guard$;
