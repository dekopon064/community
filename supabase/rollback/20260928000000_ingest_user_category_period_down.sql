-- Fail closed once a human category or copied candidate/public category exists.
begin;
set transaction isolation level read committed;
lock table machimoa_review.source_items,
  machimoa_review.processing_jobs,
  machimoa_review.source_item_user_categories,
  machimoa_review.source_item_event_periods,
  machimoa_review.curation_candidates,
  public.curations in access exclusive mode;

do $block$
begin
  if exists (select 1 from machimoa_review.source_item_user_categories)
     or exists (select 1 from machimoa_review.source_item_event_periods)
     or exists (
       select 1 from machimoa_review.curation_candidates
       where user_category is not null
          or event_start_on is not null or event_end_on is not null
     )
     or exists (
       select 1 from public.curations
       where user_category is not null
          or event_start_on is not null or event_end_on is not null
     ) then
    raise exception 'rollback_user_category_period_data_present';
  end if;
end
$block$;
...
drop function public.resolve_source_item_user_category(
  pg_catalog.uuid, pg_catalog.text, pg_catalog.text, pg_catalog.date,
  pg_catalog.date, pg_catalog.text
);
drop function machimoa_review.resolve_source_item_user_category(
  pg_catalog.uuid, pg_catalog.text, pg_catalog.text, pg_catalog.date,
  pg_catalog.date, pg_catalog.text
);
drop function machimoa_review.category_period_ready(pg_catalog.uuid, pg_catalog.text);
drop function machimoa_review.append_content_review_reason(pg_catalog.jsonb, pg_catalog.text);

alter table public.curations
  drop constraint curations_user_category_period_ck,
  drop column user_category,
  drop column event_start_on,
  drop column event_end_on;
alter table machimoa_review.curation_candidates
  drop constraint curation_candidates_user_category_period_ck,
  drop column user_category,
  drop column event_start_on,
  drop column event_end_on;
drop table machimoa_review.source_item_event_periods;
drop table machimoa_review.source_item_user_categories;
