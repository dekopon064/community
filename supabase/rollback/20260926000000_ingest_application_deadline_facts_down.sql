-- Restore origin/main function bodies for application deadline facts.
-- Does not delete review, decision, job, candidate, or curation rows.
-- Fails closed when a deadline fact or a non-null deadline value already exists.

begin;

lock table machimoa_review.source_item_application_deadlines in access exclusive mode;
lock table machimoa_review.curation_candidates in access exclusive mode;
lock table public.curations in access exclusive mode;

do $guard$
begin
  if exists (
    select 1
    from machimoa_review.source_item_application_deadlines
  ) then
    raise exception 'rollback_application_deadline_data_present';
  end if;
  if exists (
    select 1
    from machimoa_review.curation_candidates as c
    where c.application_deadline_kind is not null
       or c.application_deadline_on is not null
  ) then
    raise exception 'rollback_application_deadline_data_present';
  end if;
  if exists (
...
drop function public.resolve_source_item_application_deadline(
  pg_catalog.uuid, pg_catalog.text, pg_catalog.text, pg_catalog.date, pg_catalog.text
);
drop function machimoa_review.resolve_source_item_application_deadline(
  pg_catalog.uuid, pg_catalog.text, pg_catalog.text, pg_catalog.date, pg_catalog.text
);
drop function machimoa_review.record_source_item_application_deadline(
  pg_catalog.uuid, pg_catalog.text, pg_catalog.jsonb
);
drop function machimoa_review.append_application_deadline_unknown(pg_catalog.jsonb);

alter table machimoa_review.curation_candidates
  drop constraint curation_candidates_application_deadline_ck,
  drop column application_deadline_kind,
  drop column application_deadline_on;
alter table public.curations
  drop constraint curations_application_deadline_ck,
  drop column application_deadline_kind,
  drop column application_deadline_on;
drop table machimoa_review.source_item_application_deadlines;
