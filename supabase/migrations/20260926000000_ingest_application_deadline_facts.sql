...
-- Adds nullable columns only. Existing candidate, curation, review, and job rows stay unchanged.

begin;

create table machimoa_review.source_item_application_deadlines (
  source_item_id pg_catalog.uuid not null
    references machimoa_review.source_items (id)
    on delete restrict,
  revision_hash pg_catalog.text not null,
  application_deadline_kind pg_catalog.text not null,
  ...
  constraint source_item_application_deadlines_hash_ck
    check (revision_hash ~ '^[0-9a-f]{64}$'),
  constraint source_item_application_deadlines_pair_ck
    check (
      (
        application_deadline_kind = 'fixed'
        and application_deadline_on is not null
      )
      or (
        application_deadline_kind in ('none', 'closed')
        and application_deadline_on is null
      )
    )
);

alter table machimoa_review.source_item_application_deadlines
  owner to postgres;
alter table machimoa_review.source_item_application_deadlines
  enable row level security;
revoke all privileges on table machimoa_review.source_item_application_deadlines
  from public, anon, authenticated, service_role;

alter table machimoa_review.curation_candidates
  add column application_deadline_kind pg_catalog.text,
  add column application_deadline_on pg_catalog.date,
  add constraint curation_candidates_application_deadline_ck
    check (
      (
        application_deadline_kind is null
        and application_deadline_on is null
      )
      or (
        application_deadline_kind = 'fixed'
...
alter table public.curations
  add column application_deadline_kind pg_catalog.text,
  add column application_deadline_on pg_catalog.date,
  add constraint curations_application_deadline_ck
    check (
      (
        application_deadline_kind is null
        and application_deadline_on is null
...
create function machimoa_review.append_application_deadline_unknown(
...
begin
  ...
  if pg_catalog.jsonb_array_length(v_jobs) > 0 then
    for v_index in 0 .. pg_catalog.jsonb_array_length(v_jobs) - 1 loop
      ...
      if pg_catalog.btrim(coalesce(v_job ->> 'stage', '')) = 'content_review' then
        ...
        if not exists (
          select 1
          from pg_catalog.jsonb_array_elements_text(v_reasons) as code
          where code = 'application_deadline_unknown'
        ) then
  ...
  return v_jobs || pg_catalog.jsonb_build_array(
    pg_catalog.jsonb_build_object(
      ...
      'reason_codes', pg_catalog.jsonb_build_array('application_deadline_unknown')
    )
  );
end
$function$;

create function machimoa_review.record_source_item_application_deadline(
  p_source_item_id pg_catalog.uuid,
  p_revision_hash pg_catalog.text,
  p_deadline pg_catalog.jsonb
)
...
declare
  ...
  v_on_text pg_catalog.text;
  v_on pg_catalog.date;
begin
  if p_deadline is null or p_deadline = 'null'::pg_catalog.jsonb then
    return;
  end if;
  if pg_catalog.jsonb_typeof(p_deadline) is distinct from 'object' then
    raise exception 'invalid_application_deadline';
  end if;
  v_kind := pg_catalog.btrim(coalesce(p_deadline ->> 'kind', ''));
  v_on_text := nullif(pg_catalog.btrim(coalesce(p_deadline ->> 'on', '')), '');
  if v_kind = 'fixed' then
    if v_on_text is null then
      raise exception 'invalid_application_deadline';
    end if;
    v_on := v_on_text::pg_catalog.date;
  elsif v_kind in ('none', 'closed') then
    if v_on_text is not null then
      raise exception 'invalid_application_deadline';
    end if;
    v_on := null;
  else
  ...
  values (p_source_item_id, v_hash, v_kind, v_on)
  on conflict (source_item_id, revision_hash) do nothing;
end
$function$;

create function machimoa_review.resolve_source_item_application_deadline(
  p_source_item_id pg_catalog.uuid,
  p_revision_hash pg_catalog.text,
...
create function public.resolve_source_item_application_deadline(
  p_source_item_id pg_catalog.uuid,
  p_revision_hash pg_catalog.text,
  p_application_deadline_kind pg_catalog.text,
  p_application_deadline_on pg_catalog.date,
...
begin
  return query
  select *
  from machimoa_review.resolve_source_item_application_deadline(
    p_source_item_id,
    p_revision_hash,
    p_application_deadline_kind,
    p_application_deadline_on,
    p_reviewer
  );
end
...
create or replace function machimoa_review.sync_source_item_evaluation_jobs(
  p_source_item_id pg_catalog.uuid,
  p_revision_hash pg_catalog.text,
  p_jobs pg_catalog.jsonb
)
  ...
  loop
    ...
    if v_wanted then
      if not found then
        ...
        where j.id = v_job.id;
      elsif v_job.status = 'queued'
         or (
           v_job.status = 'claimed'
           and v_job.claim_lease_until is not null
           and v_job.claim_lease_until <= v_now
         ) then
        update machimoa_review.processing_jobs as j
        set reason_codes = v_reasons
        where j.id = v_job.id
          and j.reason_codes is distinct from v_reasons;
      end if;
...
begin
  ...
  if v_eval.disposition is distinct from 'non_target'
     and not exists (
       select 1
       from machimoa_review.source_item_application_deadlines as dl
       where dl.source_item_id = v_item.id
         and dl.revision_hash is not distinct from v_hash
     ) then
    v_eval.jobs := machimoa_review.append_application_deadline_unknown(v_eval.jobs);
  end if;
  ...
  with picked as (
    ...
    where j.processing_stage = 'ai_enrichment'
      ...
      and exists (
        select 1
        from machimoa_review.source_item_application_deadlines as dl
        where dl.source_item_id = j.source_item_id
          and dl.revision_hash is not distinct from j.revision_hash
      )
      and not exists (
        select 1
        from machimoa_review.processing_jobs as r
...
begin
  ...
  select dl.application_deadline_kind, dl.application_deadline_on
    into v_deadline_kind, v_deadline_on
  from machimoa_review.source_items as si
  join machimoa_review.source_item_application_deadlines as dl
  ...
  if not found or v_deadline_kind is null then
...
begin
  ...
  if v_deadline_kind is null
     ...
     or (
       v_deadline_kind = 'fixed' and v_deadline_on is null
     )
     or (
       v_deadline_kind in ('none', 'closed') and v_deadline_on is not null
     ) then
    raise exception 'application_deadline_required';
  end if;
...
alter function machimoa_review.append_application_deadline_unknown(pg_catalog.jsonb)
  owner to postgres;
alter function machimoa_review.record_source_item_application_deadline(
  pg_catalog.uuid, pg_catalog.text, pg_catalog.jsonb
) owner to postgres;
alter function machimoa_review.resolve_source_item_application_deadline(
  pg_catalog.uuid, pg_catalog.text, pg_catalog.text, pg_catalog.date, pg_catalog.text
) owner to postgres;
alter function public.resolve_source_item_application_deadline(
  pg_catalog.uuid, pg_catalog.text, pg_catalog.text, pg_catalog.date, pg_catalog.text
) owner to postgres;

revoke all privileges on function machimoa_review.append_application_deadline_unknown(
  pg_catalog.jsonb
) from public, anon, authenticated, service_role;
revoke all privileges on function machimoa_review.record_source_item_application_deadline(
  pg_catalog.uuid, pg_catalog.text, pg_catalog.jsonb
) from public, anon, authenticated, service_role;
revoke all privileges on function machimoa_review.resolve_source_item_application_deadline(
  pg_catalog.uuid, pg_catalog.text, pg_catalog.text, pg_catalog.date, pg_catalog.text
) from public, anon, authenticated, service_role;
revoke all privileges on function public.resolve_source_item_application_deadline(
  pg_catalog.uuid, pg_catalog.text, pg_catalog.text, pg_catalog.date, pg_catalog.text
) from public, anon, authenticated, service_role;
grant execute on function public.resolve_source_item_application_deadline(
  pg_catalog.uuid, pg_catalog.text, pg_catalog.text, pg_catalog.date, pg_catalog.text
) to service_role;
