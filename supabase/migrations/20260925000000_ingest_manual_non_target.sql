-- Manual non-target exclude for an open content_review or product_type_review.
-- decision=reject and reason_codes containing manual_non_target.
-- Memo is stored and is not interpreted. Pending current-revision candidates
-- are rejected with the existing review_status and audit columns.
-- Published candidates and publications stay up; the call fails closed.
-- Reuses public.resolve_ingest_review_decision. No new table, column, RPC, or grant.
-- Do not edit 20260924000000_ingest_content_review_close.sql.
-- Non-destructive rollback:
--   supabase/rollback/20260925000000_ingest_manual_non_target_down.sql
...
declare
  ...
  v_manual pg_catalog.bool := false;
  v_publication machimoa_review.source_publications%rowtype;
  v_candidate_id pg_catalog.uuid;
begin
  ...
  if v_type not in ('region', 'relevance', 'content', 'product_type') then
    raise exception 'invalid_review_type';
  end if;
  if v_decision not in ('approve_ai', 'reject', 'needs_review') then
    raise exception 'invalid_decision';
  end if;
  if v_type in ('content', 'product_type')
     and v_decision is distinct from 'reject' then
    raise exception 'invalid_decision';
  end if;
  if v_type = 'product_type'
     and not (
       'manual_non_target' = any(coalesce(v_reasons, '{}'::pg_catalog.text[]))
     ) then
    raise exception 'manual_non_target_required';
  end if;
  if v_type = 'content'
     and not (
       'manual_non_target' = any(coalesce(v_reasons, '{}'::pg_catalog.text[]))
       or 'insufficient_evidence' = any(coalesce(v_reasons, '{}'::pg_catalog.text[]))
     ) then
  ...
  v_manual :=
    ...
    and 'manual_non_target' = any(v_reasons);

  if v_manual then
    v_review_stage := case v_type
      when 'content' then 'content_review'
      when 'product_type' then 'product_type_review'
      else null
    end;
    ...
    if not found then
      ...
      raise exception 'product_type_review_not_open';
    end if;
    ...
    if exists (
      ...
      raise exception 'candidate_already_published';
    end if;

    select *
      into v_publication
    from machimoa_review.source_publications as p
    where p.source_id = v_item.source_id
      and p.external_key = v_item.external_key
    for update;
    if found
       and v_publication.publication_status = 'published'
       and v_publication.revision_hash is not distinct from v_hash then
      raise exception 'candidate_already_published';
    end if;
  elsif v_type = 'content' and v_decision = 'reject' then
    select *
      into v_review
    from machimoa_review.processing_jobs as j
    where j.source_item_id = v_item.id
  ...
  if v_decision = 'approve_ai' then
    ...
    if v_manual then
      ...
      set
        ...
        review_notes = case
          when v_memo is null then c.review_notes
          else v_memo
        end,
        reviewed_at = v_now,
