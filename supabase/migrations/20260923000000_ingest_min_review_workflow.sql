-- Minimum V1 review workflow: one content_review stage from evaluate_source_item_gates,
-- living_guide on human product-type confirm, and no content event_program lock.
-- Replaces existing function bodies only. No new tables, RPCs, or review/decision DML.
-- Do not apply this file without a separate operating-database approval.
-- Non-destructive rollback:
--   supabase/rollback/20260923000000_ingest_min_review_workflow_down.sql
-- Public adapters keep their signatures. Private execute stays revoked.
--
...
begin
  if current_user <> 'postgres' then
    raise exception
      'ingest min review workflow migration must run as postgres (current_user=%)',
      current_user;
  end if;

  if session_user not in ('postgres', 'cli_login_postgres') then
    raise exception
      'ingest min review workflow session_user not allowed (%)',
      session_user;
  end if;
...
begin
  ...
  if p_has_source_url is not true then
    ...
    jobs := pg_catalog.jsonb_build_array(
      pg_catalog.jsonb_build_object(
        'stage', 'content_review',
        'reason_codes', pg_catalog.jsonb_build_array('missing_source_url')
      )
    );
    region_status := 'not_applicable';
    audience_status := 'not_applicable';
  ...
  if v_region = 'failed' or v_audience = 'failed' then
    ...
    if v_region = 'review_required' then
      v_reasons := v_reasons || pg_catalog.jsonb_build_array('region_scope_unknown');
    end if;
    if v_audience = 'review_required' then
      v_reasons := v_reasons || pg_catalog.jsonb_build_array('relevance_unconfirmed');
    end if;
    if v_reasons <> '[]'::pg_catalog.jsonb then
      v_disp := 'region_review_required';
      v_jobs := pg_catalog.jsonb_build_array(
        pg_catalog.jsonb_build_object(
          'stage', 'content_review',
          'reason_codes', v_reasons
        )
...
begin
  ...
  if v_action in ('confirm', 'override')
     and v_type not in ('event_program', 'policy_reference', 'living_guide') then
  ...
  if v_kind = 'no-op' then
    ...
    if v_action = 'confirm' then
      ...
      where pt.source_item_id = v_item.id
        and pt.revision_hash = v_hash;
      if machimoa_review.gate_facts_row_is_complete_v1(
